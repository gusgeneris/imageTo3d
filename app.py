...
parser.add_argument("--stage2_config", type=str, default="configs/stage2-v2-snr.yaml", help="config for stage2")
parser.add_argument("--device", type=str, default="cuda")
args = parser.parse_args()
...
with gr.Blocks() as demo:
    ...
    with gr.Row():
        with gr.Column():
            gr.Markdown("La generación del modelo 3D se realizó automáticamente.")
        with gr.Column():
            image_output = gr.Image(value=processed_image, interactive=False, label="Output RGB image")
            xyz_output = gr.Image(value=np_xyzs, interactive=False, label="Output CCM image")
